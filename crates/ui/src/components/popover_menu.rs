...
impl<M: ManagedView> PopoverMenu<M> {
    ...
    fn resolved_attach(&self) -> Corner {
        match self.resolved_attach_anchor() {
            Anchor::TopLeft => Corner::TopLeft,
            Anchor::TopCenter | Anchor::TopRight => Corner::TopRight,
            Anchor::BottomLeft | Anchor::BottomCenter => Corner::BottomLeft,
            Anchor::BottomRight => Corner::BottomRight,
            Anchor::LeftCenter | Anchor::RightCenter => Corner::TopLeft, // 随便映射一个
        }
    }
    fn resolved_attach_anchor(&self) -> Anchor {
        self.attach
