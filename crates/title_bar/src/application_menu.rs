...
use gpui::{
    Action, App, Context, CursorStyle, Div, Entity, InteractiveElement, IntoElement, OwnedMenu,
    OwnedMenuItem, ParentElement, Render, SharedString, StatefulInteractiveElement, Styled, Window,
    actions, div, prelude::FluentBuilder,
};
use gpui_component::{Selectable, button::ButtonVariants, menu::PopupMenu, tooltip::Tooltip};
use settings::Settings;
use smallvec::SmallVec;
use ui::{
    Button, ButtonCommon as _, ButtonStyle, IconButton, LabelSize, PopoverMenu, PopoverMenuHandle,
    clickable::Clickable,
...
struct MenuEntry {
    menu: OwnedMenu,
    handle: PopoverMenuHandle<PopupMenu>,
}
...
impl ApplicationMenu {
    ...
    fn build_menu_from_items(
        ...
        PopupMenu::build(window, cx, |menu, window, cx| {
            ...
            let menu = menu.when_some(window.focused(cx), |menu, focused| {
                menu.action_context(focused)
            });
            ...
            sanitized_items
                ...
                .fold(menu, |menu, item| match item {
                    ...
                    OwnedMenuItem::Action { name, action, .. } => menu.menu(name, action),
                    OwnedMenuItem::Submenu(submenu) => {
                        submenu
                            .items
                            .into_iter()
            ...
            .child(
                PopoverMenu::new(SharedString::from(format!("{}-menu-popover", menu_name)))
                    ...
                    .trigger_with_tooltip(
                        ...
                        |window, cx| Tooltip::new("Open Application Menu").build(window, cx),
                    )
                    .with_handle(handle),
            )
    }
...
impl Render for ApplicationMenu {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        // 紧凑	!all_menus_shown	一个汉堡按钮，点击后显示所有菜单
        // 展开	all_menus_shown	每个菜单都是独立按钮（文件/编辑/选择...）,悬停切换
